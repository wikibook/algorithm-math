...
// 기반이 되는 등산 알고리즘의 구현은 answer_A50.java를 참고하시기 바랍니다.
// ※ 예상 점수는 약 99.977억 점입니다.
...
class Main {
	public static void main(String[] args) {
		...
		while ((System.currentTimeMillis() - ti) / 1000.0 < TIME_LIMIT) {
			...
			int old_x = ans.X[t], new_x = ans.X[t] + (rnd.nextInt(3) - 1);  // X[t]를 -1에서 +1까지 증감
			int old_y = ans.Y[t], new_y = ans.Y[t] + (rnd.nextInt(3) - 1);  // Y[t]를 -1에서 +1까지 증감
			int old_z = ans.H[t], new_z = ans.H[t] + (rnd.nextInt(h_limit * 2 + 1) - h_limit); // H[t]를 -h_limit에서 +h_limit으로 증가 또는 감소
